        ...
        public async Task GenerateNode(Node node, GameObject parent, IList<NodeTreeElement> nodeTreeElements)
        {
            ...
            if (treeElement.actionType != ActionType.None)
            {
                ...
                if (nodeGo == null)
                {
                    ...
                    if (isParentCanvas)
                        root = parentT;
                    nodeGo.name = $"{node.name}_[{node.id}]";
                ...
                if (!isParentCanvas)
                    ...
                    TransformUtils.SetConstraints(parentT, rectTransform, new Constraints(){horizontal = "CENTER", vertical = "CENTER"});
                //ImageUtils.SetMask(node, nodeGo);
            }

            string nodeName = node.name;
            bool isGroup = node.type == "FRAME" || node.type == "GROUP";
            
            if (node.type == "VECTOR" || node.type == "RECTANGLE" || nodeName.Contains("_img"))
            {
                await ImageUtils.RenderNodeAndApply(node, nodeGo, _importer);
            }
            
            if(nodeName.Contains("_btn"))
            {
                nodeGo.AddComponent<Button>();
            }
            else if (nodeName.Contains("_txt") || node.type == "TEXT")
            {
                AddText(node, nodeGo);
            }
            
            if (node.children == null || (isGroup && nodeName.Contains("_img")))
            {
                return;
            }
