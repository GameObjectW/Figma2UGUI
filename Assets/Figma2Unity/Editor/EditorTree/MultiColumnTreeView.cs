...
namespace FigmaImporter.Editor.EditorTree
{
    internal class MultiColumnTreeView : TreeViewWithTreeModel<NodeTreeElement>
    {
        ...
        void CellGUI(Rect cellRect, TreeViewItem<NodeTreeElement> item, MyColumns column, ref RowGUIArgs args)
        {
            ...
            switch (column)
            {
                case MyColumns.Name:
                {
                    ...
                    {
                        EditorGUI.showMixedValue = item.children.Exists(child =>
                        {
                            if (child == null)
                            {
                                return false;
                            ...
                            TreeViewItem<NodeTreeElement> childItem = (TreeViewItem<NodeTreeElement>) child;
                            return childItem.data.enabled != item.data.enabled;
                        });
                    
                        EditorGUI.BeginChangeCheck();
                        bool newEnabled = EditorGUI.Toggle(toggleRect, item.data.enabled);
                        if (EditorGUI.EndChangeCheck())
                        {
                            item.data.enabled = newEnabled;
                            SetChildrenEnabled(item, newEnabled);
                        }
        ...
        void SetChildrenEnabled(TreeViewItem<NodeTreeElement> parent, bool enabled)
        {
            ...
            {
                var child = (TreeViewItem<NodeTreeElement>)treeViewItem;
                if (child == null)
                {
                    continue;
                }
                child.data.enabled = enabled;
