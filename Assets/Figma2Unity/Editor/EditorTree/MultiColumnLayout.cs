...
namespace FigmaImporter.Editor.EditorTree
{
	...
	class MultiColumnLayout
	{
		...
		IList<NodeTreeElement> GetData(IList<Node> nodes, ref int idCounter, int depth = -1)
		{
			...
			if (currentDepth == -1)
			{
				result.Add(new NodeTreeElement("Root", "Root", ActionType.None, null, currentDepth, idCounter, true));
				idCounter++;
				currentDepth++;
			}
			...
			{
				result.Add(new NodeTreeElement(node.name,node.id, ActionType.None, null, currentDepth, idCounter, !node.visible));
				idCounter++;
